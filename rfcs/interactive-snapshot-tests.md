...
- Both flavors run in CI from day one. The `cli-snap-test` job (which builds `packages/cli/dist` and installs the release binary) runs the full suite once per OS, pointing the global flavor at the installed binary via `VP_SNAP_GLOBAL_VP` so no second `vite_global_cli` compile is needed. The Rust `test` job additionally runs a fast global-only leg (`VP_SNAP_SKIP_FLAVORS=local`, since it has a toolchain but no built JS CLI) for early signal.
- The Windows story reuses the existing cross-compile infrastructure: build the test archive on Linux with cargo-xwin plus `cargo nextest archive` (with the `vp` binary added via nextest's `archive.include`, since it is not a test binary), run partitions on Windows runners. Local-flavor cases additionally need node and the built `packages/cli/dist` on the runner, which the current Windows snap job already provisions.
