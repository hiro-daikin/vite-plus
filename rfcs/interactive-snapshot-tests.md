...
- Staged rollout: the existing Rust `test` job runs `just snapshot-test` with `VP_SNAP_SKIP_FLAVORS=local` (it has a toolchain but no built JS CLI). Local-flavor coverage joins CI when the old snap-test job is replaced during migration, on runners that already build `packages/cli/dist`.
